...
func (h *AssessmentHandler) markNotificationRead(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.db.First(&notification, "id = ? AND recipient_id = ?", id, claims.ID).Error; err != nil {
...
func (h *AssessmentHandler) markAllNotificationsRead(w http.ResponseWriter, r *http.Request) {
	...
	now := nowPtr()
	if err := h.db.Model(&models.Notification{}).Where("recipient_id = ? AND read_at IS NULL", claims.ID).Update("read_at", now).Error; err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *AssessmentHandler) createNotificationAPI(w http.ResponseWriter, r *http.Request) {
	if !h.requireAnyRole(w, r, "admin", "dlc_director", "hod", "exam_officer") {
		return
	}
	...
	if payload.RecipientID == uuid.Nil || strings.TrimSpace(payload.Title) == "" {
		writeError(w, http.StatusBadRequest, "recipient_id and title are required")
		return
	}
...
func (h *AssessmentHandler) createNotification(recipientID uuid.UUID, senderID *uuid.UUID, title string, message string, category string, priority string, actionURL string, resourceType string, resourceID *uuid.UUID) {
	if recipientID == uuid.Nil || strings.TrimSpace(title) == "" {
		return
	}
	notification := models.Notification{
