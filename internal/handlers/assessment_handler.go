...
func (h *AssessmentHandler) RegisterRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /api/student/activity-records", h.createExamActivityRecord)
	mux.HandleFunc("POST /api/proctoring/events", h.createExamActivityRecord)
	...
	mux.HandleFunc("GET /api/review/evidence-cases", h.listReviewEvidenceCases)
	mux.HandleFunc("POST /api/review/evidence-cases", h.createReviewEvidenceCase)
	mux.HandleFunc("POST /api/review/evidence-cases/", h.reviewEvidenceCaseAction)
