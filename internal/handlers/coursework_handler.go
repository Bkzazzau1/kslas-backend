...
func (h *AssessmentHandler) listLecturerAssignments(w http.ResponseWriter, r *http.Request) {
	if !h.requireAnyRole(w, r, "lecturer", "exam_officer", "hod", "dlc_director", "admin") {
		return
	}
...
func (h *AssessmentHandler) createLecturerAssignment(w http.ResponseWriter, r *http.Request) {
	if !h.requireAnyRole(w, r, "lecturer") {
		return
	}
	...
	if !h.lecturerHasActiveCourseAssignment(claims.ID, assignment.CourseID) {
...
func (h *AssessmentHandler) assignmentAction(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		writeError(w, http.StatusNotFound, "invalid assignment action")
		return
	}
	var assignment models.LecturerAssignment
...
func (h *AssessmentHandler) listAssignmentSubmissions(w http.ResponseWriter, r *http.Request) {
	if !h.requireAnyRole(w, r, "lecturer", "hod", "dlc_director", "admin") {
		return
	}
...
func (h *AssessmentHandler) markAssignmentSubmission(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.db.Preload("Assignment").First(&submission, "id = ?", id).Error; err != nil {
	...
	if submission.Assignment.FeedbackEnabled {
		submission.FeedbackReleasedAt = nowPtr()
	}
...
func (h *AssessmentHandler) submitCA(w http.ResponseWriter, r *http.Request) {
	...
	if err := decodeJSON(w, r, &ca); err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	ca.LecturerID = claims.ID
	if !h.lecturerHasActiveCourseAssignment(claims.ID, ca.CourseID) {
		writeError(w, http.StatusForbidden, "lecturer is not assigned to this course")
		return
	}
...
func (h *AssessmentHandler) submitMarkedExamScripts(w http.ResponseWriter, r *http.Request) {
	...
	if !h.lecturerHasActiveCourseAssignment(claims.ID, item.CourseID) {
		writeError(w, http.StatusForbidden, "lecturer is not assigned to this course")
		return
	}
